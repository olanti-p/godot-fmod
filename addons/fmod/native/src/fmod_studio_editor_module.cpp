...
void FMODSettings::initialize() {
    ...
    if (Engine::get_singleton()->is_editor_hint()) {
        ...
        if (error) {
            UtilityFunctions::push_error(String("Encountered error {0} when saving FMOD Project Settings").format(Array::make(error)));
        } else {
...
void FMODSettings::add_fmod_settings() {
    ...
    add_setting(
        settings_path + String("fmod_output_type"),
        0,
        Variant::Type::INT,
        PROPERTY_HINT_ENUM,
    ...
    add_setting(settings_path + String("fmod_debug_flags"), 0, Variant::Type::INT, PROPERTY_HINT_FLAGS, "Error,Warning,Log");
    add_setting(settings_path + String("fmod_debug_mode"), 1, Variant::Type::INT, PROPERTY_HINT_ENUM, "TTY,File,Godot Output");
    add_setting(
        settings_path + String("banks_to_load_at_startup"),
        TypedArray<String>(),
        Variant::Type::ARRAY,
    ...
    add_setting(settings_path + String("banks_flags"), 0, Variant::Type::INT, PROPERTY_HINT_FLAGS, "Non Blocking,Decompress Samples,Unencrypted");
    add_setting(settings_path + String("sample_rate"), 48000, Variant::Type::INT);
    add_setting(settings_path + String("speaker_mode"), 3, Variant::Type::INT, PROPERTY_HINT_ENUM, "Default,Raw,Mono,Stereo,Quad,Surround,5.1,7.1,7.1.4");
...
void FMODSettings::add_setting(const String& name, const Variant& default_value, Variant::Type type, PropertyHint hint, const String& hint_string, int usage) {
    Dictionary setting;
    setting["name"] = name;
    setting["type"] = type;
    setting["hint"] = hint;
...
void FMODStudioEditorModule::_bind_methods() {
    ClassDB::bind_static_method("FMODStudioEditorModule", D_METHOD("get_singleton"), &FMODStudioEditorModule::get_singleton);
    ...
    ClassDB::bind_method(D_METHOD("create_tree_items", "root", "items", "parent"), &FMODStudioEditorModule::create_tree_items);
    ...
    ClassDB::bind_method(D_METHOD("set_preview_parameter", "parameter_name", "value"), &FMODStudioEditorModule::set_preview_parameter);
...
bool FMODStudioEditorModule::initialize_fmod() {
    ...
    if (!ERROR_CHECK(core_system->setFileSystem(
            &FMODGodotBlockingIO::file_open, &FMODGodotBlockingIO::file_close, &FMODGodotBlockingIO::file_read, &FMODGodotBlockingIO::file_seek, 0, 0, -1
        ))) {
        return false;
    }
    ...
    const unsigned int sample_rate = static_cast<unsigned int>(get_platform_project_setting(settings_path + String("sample_rate")));
    const unsigned int speaker_mode = static_cast<unsigned int>(get_platform_project_setting(settings_path + String("speaker_mode")));

    if (!ERROR_CHECK(core_system->setSoftwareFormat(sample_rate, static_cast<FMOD_SPEAKERMODE>(speaker_mode), 0))) {
        return false;
    }

    const unsigned int buffer_length = static_cast<unsigned int>(get_platform_project_setting(settings_path + String("dsp_buffer_length")));
    ...
    const int real_channels = static_cast<int>(get_platform_project_setting(settings_path + String("real_channel_count")));
    ...
    const String encryption_key = static_cast<String>(get_platform_project_setting(settings_path + String("encryption_key")));
    ...
    const int virtual_channels = static_cast<int>(get_platform_project_setting(settings_path + String("virtual_channel_count")));
    ...
    AudioUnitGetProperty(audio_unit, kAudioOutputUnitProperty_CurrentDevice, kAudioUnitScope_Global, 0, &device_id, &audio_device_id_size);
    ...
    AudioDeviceSetProperty(device_id, NULL, 0, FALSE, kAudioDevicePropertyBufferFrameSize, sizeof(buffer_frame_size), &buffer_frame_size);
#endif
    ...
    if (DirAccess::get_open_error() == godot::Error::OK) {
        ...
        while (file_name != "") {
            if (file_name.ends_with(".bank")) {
                ...
                Ref<FileAccess> file = FileAccess::open(bank_path + String("/") + file_name, FileAccess::ModeFlags::READ);
...
void FMODStudioEditorModule::load_all_banks() {
    ...
    if (bank_files_infos.size() == 0) {
        ...
        UtilityFunctions::push_warning(
            "[FMOD] No Banks found in the banks folder. Add a path to a folder containing Banks in the FMOD Project Settings and refresh the Project."
        );
        return;
    }
    ...
    if (index == -1) {
        first_run = false;
        UtilityFunctions::push_warning("[FMOD] No .strings bank found in the bank path. Check the Banks Path setting in the Project Settings and make sure to "
                                       "export a .strings Master Bank.");
        return;
    }
...
void FMODStudioEditorModule::reload_cache_file() {
    project_cache = ResourceLoader::get_singleton()->load(
        "res://addons/fmod/editor/cache/fmod_project_cache.tres", "ProjectCache", ResourceLoader::CacheMode::CACHE_MODE_REPLACE
    );
}
            ...
            if (event_count > 0) {
                ...
                for (int64_t i = 0; i < events_array.size(); i++) {
                    ...
                    if (!FileAccess::file_exists(resource_dirs["events"].operator godot::String() + guid + ".tres")) {
                        ...
                        Ref<EventAsset> asset = ResourceLoader::get_singleton()->load(
                            resource_dirs["events"].operator godot::String() + guid + ".tres", "EventAsset"
                        );
                ...
                for (int64_t i = 0; i < snapshots_array.size(); i++) {
                    ...
                    if (!FileAccess::file_exists(resource_dirs["snapshots"].operator godot::String() + guid + ".tres")) {
                        ...
                        Ref<EventAsset> asset = ResourceLoader::get_singleton()->load(
                            resource_dirs["snapshots"].operator godot::String() + guid + ".tres", "EventAsset"
                        );
            ...
            if (bus_count > 0) {
                ...
                for (int64_t i = 0; i < bus_infos.size(); i++) {
                    ...
                    if (!FileAccess::file_exists(resource_dirs["busses"].operator godot::String() + guid + ".tres")) {
                        ...
                        Ref<BusAsset> asset = ResourceLoader::get_singleton()->load(
                            resource_dirs["busses"].operator godot::String() + guid + ".tres", "BusAsset"
                        );
            ...
            if (vca_count > 0) {
                ...
                for (int64_t i = 0; i < vca_infos.size(); i++) {
                    ...
                    if (!FileAccess::file_exists(resource_dirs["vcas"].operator godot::String() + guid + ".tres")) {
                        ...
                        Ref<VCAAsset> asset = ResourceLoader::get_singleton()->load(
                            resource_dirs["vcas"].operator godot::String() + guid + ".tres", "VCAAsset"
                        );
        ...
        if (parameter_count > 0) {
            ...
            for (int64_t i = 0; i < parameter_infos.size(); i++) {
                ...
                if (!FileAccess::file_exists(resource_dirs["parameters"].operator godot::String() + guid + ".tres")) {
                    ...
                    Ref<ParameterAsset> asset = ResourceLoader::get_singleton()->load(
                        resource_dirs["parameters"].operator godot::String() + guid + ".tres", "ParameterAsset"
                    );
...
bool FMODStudioEditorModule::has_event_changed(const Ref<EventAsset>& old_event, const Ref<EventAsset>& to_check) {
    ...
    if (old_name != to_check->get_name() || old_path != to_check->get_path() || old_3d != to_check->get_3d() || old_oneshot != to_check->get_oneshot() ||
        old_max_distance != to_check->get_max_distance() || old_min_distance != to_check->get_min_distance()) {
        return true;
    }
...
void ProjectCache::_bind_methods() {
    ...
    ADD_PROPERTY(PropertyInfo(Variant::DICTIONARY, "banks", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT), "set_banks", "get_banks");
    ADD_PROPERTY(PropertyInfo(Variant::DICTIONARY, "events", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT), "set_events", "get_events");
    ADD_PROPERTY(PropertyInfo(Variant::DICTIONARY, "snapshots", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT), "set_snapshots", "get_snapshots");
    ADD_PROPERTY(PropertyInfo(Variant::DICTIONARY, "vcas", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT), "set_vcas", "get_vcas");
    ADD_PROPERTY(PropertyInfo(Variant::DICTIONARY, "busses", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT), "set_busses", "get_busses");
    ...
    ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "bank_tree", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_STORAGE), "set_bank_tree", "get_bank_tree");
    ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "event_tree", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_STORAGE), "set_event_tree", "get_event_tree");
    ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "snapshot_tree", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_STORAGE), "set_snapshot_tree", "get_snapshot_tree");
    ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "vca_tree", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_STORAGE), "set_vca_tree", "get_vca_tree");
    ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "bus_tree", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_STORAGE), "set_bus_tree", "get_bus_tree");
    ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "parameter_tree", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_STORAGE), "set_parameter_tree", "get_parameter_tree");
}
