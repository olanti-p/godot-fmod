...
bool FMODStudioModule::initialize_fmod() {
    ...
    unsigned int ouput_type = static_cast<unsigned int>(get_platform_project_setting(settings_path + String("fmod_output_type")));

    if (!ERROR_CHECK(core_system->setOutput(static_cast<FMOD_OUTPUTTYPE>(ouput_type)))) {
        String warning_message = "[WARNING]: " + String("Failed to set the selected Output Type. Initializing FMOD with OUTPUTTYPE_NOSOUND.") + " " +
            __FUNCTION__ + " in " + __FILE__ + ":" + String::num(__LINE__);
    ...
    const bool enable_live_update = static_cast<bool>(get_platform_project_setting(settings_path + String("enable_live_update")));
    const unsigned short profile_port = static_cast<int>(get_platform_project_setting(settings_path + String("live_update_port")));
    ...
    const bool enable_memory_tracking = static_cast<bool>(get_platform_project_setting(settings_path + String("enable_memory_tracking")));
...
void FMODStudioModule::initialize_debug_functions() {
    ...
    unsigned int fmod_debug_flags = static_cast<unsigned int>(get_platform_project_setting(settings_path + String("fmod_debug_flags")));
    unsigned int fmod_debug_mode = static_cast<unsigned int>(get_platform_project_setting(settings_path + String("fmod_debug_mode")));

    ERROR_CHECK(FMOD::Debug_Initialize((FMOD_DEBUG_FLAGS)fmod_debug_flags, (FMOD_DEBUG_MODE)fmod_debug_mode, (FMOD_DEBUG_CALLBACK)debug_monitor, "fmod.log"));
}
...
FMODStudioModule::FMODStudioModule() {
    ...
    ERROR_CHECK(FMOD::Memory_Initialize(nullptr, 0, FMODGodotMemory::user_alloc, FMODGodotMemory::user_realloc, FMODGodotMemory::user_free, FMOD_MEMORY_ALL));
}
