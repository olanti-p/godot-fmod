...
void StudioGlobalParameterTrigger::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_parameter", "parameter"), &StudioGlobalParameterTrigger::set_parameter);
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::OBJECT, "parameter", PROPERTY_HINT_RESOURCE_TYPE, "ParameterAsset", PROPERTY_USAGE_DEFAULT), "set_parameter", "get_parameter"
    );
}
...
void StudioGlobalParameterTrigger::_get_property_list(List<PropertyInfo>* p_list) const {
    if (parameter.is_valid()) {
        ...
        float step{};
        Ref<ParameterAsset> param = parameter;
        Ref<FmodTypes::FMOD_STUDIO_PARAMETER_DESCRIPTION> parameter_description = param->get_parameter_description();

        if ((parameter_description->get_flags() & FMOD_STUDIO_PARAMETER_DISCRETE) && !(parameter_description->get_flags() & FMOD_STUDIO_PARAMETER_LABELED)) {
            flags = Variant::FLOAT;
            step = 1.0f;
...
void StudioGlobalParameterTrigger::trigger() {
    ...
    if (fmod_module) {
        if (parameter.is_valid()) {
            ...
            fmod_module->get_studio_system_ref()->set_parameter_by_name(parameter_name, parameter_value, false);
        }
    }
}
