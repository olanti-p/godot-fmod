    ...
    if (name.begins_with("parameter_")) {
        ...
        if (event.is_valid()) {
            ...
            if (p_value.get_type() == Variant::NIL) {
                ...
                if (default_parameter.is_valid()) {
                    Ref<FmodTypes::FMOD_STUDIO_PARAMETER_ID> id = default_parameter->get_parameter_description()->get_id();
...
void StudioEventEmitterImpl<T>::_get_property_list(List<PropertyInfo>* p_list) const {
    ...
    p_list->push_back(
        PropertyInfo(Variant::NIL, "Initial Parameter Values", PROPERTY_HINT_NONE, "parameter_", PROPERTY_USAGE_GROUP | PROPERTY_USAGE_SCRIPT_VARIABLE)
    );
    ...
    if (parameters.size() > 0) {
        ...
        for (int i = 0; i < values.size(); i++) {
            ...
            if (parameter->get_parameter_description()->get_flags() & FMOD_STUDIO_PARAMETER_LABELED) {
                ...
                hint_string = String(
                    String::num_real(parameter->get_parameter_description()->get_minimum()) + "," +
                    String::num_real(parameter->get_parameter_description()->get_maximum()) + "," + String::num_real(step)
                );
            }
...
void StudioEventEmitterImpl<StudioEventEmitter2D>::update_playing_status(bool force) {
    ...
    should_play_instance = ListenerImpl::distance_to_nearest_listener(transform.get_origin() / FMODStudioModule::get_singleton()->distance_scale_2d) <=
...
void StudioEventEmitter2D::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_preload_samples", "preload_samples"), &StudioEventEmitter2D::set_preload_samples);
    ...
    ClassDB::bind_method(D_METHOD("set_overridden_parameters", "overridden_parameters"), &StudioEventEmitter2D::set_overridden_parameters);
    ClassDB::bind_method(D_METHOD("get_overridden_parameters"), &StudioEventEmitter2D::get_overridden_parameters);

    ADD_PROPERTY(PropertyInfo(Variant::INT, "play_event", PROPERTY_HINT_ENUM, "None,Enter Tree,Ready,Exit Tree"), "set_play_event", "get_play_event");
    ADD_PROPERTY(PropertyInfo(Variant::INT, "stop_event", PROPERTY_HINT_ENUM, "None,Enter Tree,Ready,Exit Tree"), "set_stop_event", "get_stop_event");
    ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "event", PROPERTY_HINT_RESOURCE_TYPE, "EventAsset"), "set_event", "get_event");
...
void StudioEventEmitter3D::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_preload_samples", "preload_samples"), &StudioEventEmitter3D::set_preload_samples);
    ...
    ClassDB::bind_method(D_METHOD("set_overridden_parameters", "overridden_parameters"), &StudioEventEmitter3D::set_overridden_parameters);
