...
void StudioParameterTrigger::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_studio_event_emitter", "path"), &StudioParameterTrigger::set_studio_event_emitter);
    ...
    ADD_PROPERTY(PropertyInfo(Variant::INT, "trigger_on", PROPERTY_HINT_ENUM, "None,Enter Tree,Ready,Exit Tree"), "set_trigger_on", "get_trigger_on");
    ADD_PROPERTY(
        PropertyInfo(
            Variant::NODE_PATH, "studio_event_emitter", PROPERTY_HINT_NODE_PATH_VALID_TYPES, "StudioEventEmitter3D,StudioEventEmitter2D", PROPERTY_USAGE_DEFAULT
        ),
        "set_studio_event_emitter",
        "get_studio_event_emitter"
    );
    ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "event", PROPERTY_HINT_RESOURCE_TYPE, "EventAsset", PROPERTY_USAGE_STORAGE), "set_event", "get_event");
}
...
void StudioParameterTrigger::_get_property_list(List<PropertyInfo>* p_list) const {
    ...
    p_list->push_back(PropertyInfo(Variant::NIL, "Parameters", PROPERTY_HINT_NONE, "parameter_", PROPERTY_USAGE_GROUP | PROPERTY_USAGE_SCRIPT_VARIABLE));
    ...
    if (parameters.size() > 0) {
        ...
        for (int64_t i = 0; i < values.size(); i++) {
            ...
            Ref<FmodTypes::FMOD_STUDIO_PARAMETER_DESCRIPTION> parameter_description = parameter->get_parameter_description();
            ...
            if (parameter_description->get_flags() & FMOD_STUDIO_PARAMETER_LABELED) {
                ...
                hint_string = String(
                    String::num_real(parameter_description->get_minimum()) + "," + String::num_real(parameter_description->get_maximum()) + "," +
...
bool StudioParameterTrigger::_property_get_revert(const StringName& p_name, Variant& r_property) const {
    ...
    if (name.begins_with("parameter_")) {
        ...
        if (parameter.is_valid()) {
            Ref<FmodTypes::FMOD_STUDIO_PARAMETER_ID> id = parameter->get_parameter_description()->get_id();
        ...
        if (emitter_3d) {
            if (emitter_3d->implementation.event_instance.is_valid()) {
                for (int64_t i = 0; i < overridden_parameters.size(); i++) {
                    ...
                    emitter_3d->implementation.event_instance->set_parameter_by_name(parameter_name, parameter_value, false);
                }
            }
        } else if (emitter_2d) {
            if (emitter_2d->implementation.event_instance.is_valid()) {
                for (int64_t i = 0; i < overridden_parameters.size(); i++) {
                    ...
                    emitter_2d->implementation.event_instance->set_parameter_by_name(parameter_name, parameter_value, false);
                }
            }
        }
    }
