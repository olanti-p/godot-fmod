...
void StudioBankLoader::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_preload_sample_data", "preload_sample_data"), &StudioBankLoader::set_preload_sample_data);
    ClassDB::bind_method(D_METHOD("get_preload_sample_data"), &StudioBankLoader::get_preload_sample_data);

    ADD_PROPERTY(PropertyInfo(Variant::INT, "load_on", PROPERTY_HINT_ENUM, "None,Enter Tree,Ready,Exit Tree"), "set_load_on", "get_load_on");
    ADD_PROPERTY(PropertyInfo(Variant::INT, "unload_on", PROPERTY_HINT_ENUM, "None,Enter Tree,Ready,Exit Tree"), "set_unload_on", "get_unload_on");
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "preload_sample_data", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT), "set_preload_sample_data", "get_preload_sample_data"
    );
    ADD_PROPERTY(PropertyInfo(Variant::INT, "num_banks", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT), "set_num_banks", "get_num_banks");
}
...
void StudioBankLoader::_get_property_list(List<PropertyInfo>* p_list) const {
    ...
    for (int i = 0; i < num_banks; i++) {
        p_list->push_back(PropertyInfo(
            Variant::OBJECT,
            String("bank_" + String::num_int64(i)).utf8().get_data(),
    ...
    if (fmod_module) {
        for (int64_t i = 0; i < banks.size(); i++) {
            ...
            if (bank_ref.is_valid()) {
                fmod_module->get_studio_system_ref()->load_bank_file(bank_ref->get_file_path(), FMOD_STUDIO_LOAD_BANK_NORMAL, preload_sample_data);
            }
        }
    }
}
...
void StudioBankLoader::unload() {
    ...
    if (fmod_module) {
        for (int64_t i = 0; i < banks.size(); i++) {
            Ref<BankAsset> bank_ref = banks[i];
            Ref<StudioApi::Bank> bank = fmod_module->get_studio_system_ref()->get_bank(bank_ref->get_path());
