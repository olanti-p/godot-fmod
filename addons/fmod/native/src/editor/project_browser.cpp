...
void FMODProjectBrowserWindow::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("on_generate_guids_button_pressed"), &FMODProjectBrowserWindow::on_generate_guids_button_pressed);
...
void FMODProjectBrowserWindow::on_generate_guids_button_pressed() {
    ...
    const String script_path = ProjectSettings::get_singleton()->globalize_path("res://addons/fmod/editor/tools/export_guids.js");
    String result;
    ...
    Error err = client->get_command("fmodModule.writeHeaderFile('" + ProjectSettings::get_singleton()->globalize_path(guids_path) + "');", result);

    if (err == Error::FAILED) {
        UtilityFunctions::push_warning(
            "[FMOD] Failed to generate GUIDs. Please verify that an FMOD Studio project is open and the path to the Banks in the Project Settings is correct."
        );
    } else {
        ResourceLoader::get_singleton()->load(guids_path, "", ResourceLoader::CacheMode::CACHE_MODE_REPLACE);
