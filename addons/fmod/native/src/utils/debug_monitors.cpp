...
void FMODDebugMonitor::_enter_tree() {
    ...
    if (!performance->has_custom_monitor("FMOD/Studio CPU Update (%)")) {
        performance->add_custom_monitor("FMOD/Studio CPU Update (%)", Callable(this, "get_cpu_studio_update"));
    }
    if (!performance->has_custom_monitor("FMOD/Memory Allocation (MB)")) {
        performance->add_custom_monitor("FMOD/Memory Allocation (MB)", Callable(this, "get_current_memory_alloc"));
    }
    if (!performance->has_custom_monitor("FMOD/Max Memory Allocation (MB)")) {
        performance->add_custom_monitor("FMOD/Max Memory Allocation (MB)", Callable(this, "get_max_memory_alloc"));
    }
