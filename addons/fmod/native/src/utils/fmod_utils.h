...
static bool error_check(const FMOD_RESULT result, const char* function, const char* file, int line) {
    if (result != FMOD_OK) {
        String warning_message = "[WARNING]: " + String(FMOD_ErrorString(result)) + " " + function + " in " + file + ":" + String::num(line);
...
enum class VectorType { POSITION, FORWARD, UP, VELOCITY };
...
static inline void transform3d_to_fmod_vector(
    const Transform3D& in_transform,
    FMOD_VECTOR& out_vector,
    const VectorType& type,
    Object* physicsbody3d = nullptr
) {
    switch (type) {
...
static inline void transform2d_to_fmod_vector(
    const Transform2D& in_transform,
    FMOD_VECTOR& out_vector,
    const VectorType& type,
    Object* physicsbody2d = nullptr,
    float distance_scale_2d = 1.0f
) {
    switch (type) {
        case VectorType::POSITION:
            vector3_to_fmod_vector(
                Vector3(in_transform.get_origin().x / distance_scale_2d, (in_transform.get_origin().y * -1.0f) / distance_scale_2d, 0.0f), out_vector
            );
            break;
        case VectorType::FORWARD:
            vector3_to_fmod_vector(Vector3(in_transform.columns[1].x, 0.0f, in_transform.columns[1].y).normalized(), out_vector);
            break;
        case VectorType::UP:
        ...
        case VectorType::VELOCITY:
            if (physicsbody2d) {
                ...
                if (rigidbody2d) {
                    vector3_to_fmod_vector(Vector3(rigidbody2d->get_linear_velocity().x, rigidbody2d->get_linear_velocity().y, 0), out_vector);
                } else {
                    const CharacterBody2D* characterbody2d = Object::cast_to<CharacterBody2D>(physicsbody2d);
                    if (characterbody2d) {
                        vector3_to_fmod_vector(Vector3(characterbody2d->get_velocity().x, characterbody2d->get_velocity().y, 0), out_vector);
                    }
                }
            } else {
                vector3_to_fmod_vector(Vector3(0.0f, 0.0f, 0.0f), out_vector);
...
static inline void transform2d_to_3dattributes(
    const Transform2D& in_transform,
    FMOD_3D_ATTRIBUTES& out_attributes,
    Object* physicsbody2d = nullptr,
    float distance_scale_2d = 1.0f
) {
...
static inline void fmod_obj_to_path(const T& obj, String& out_path) {
    ...
    do {
        ...
        if (result == FMOD_ERR_EVENT_NOTFOUND) {
            UtilityFunctions::printerr("[FMOD] Failed to get the Path of an FMOD Object", __FUNCTION__, __FILE__, __LINE__);
            break;
        }
