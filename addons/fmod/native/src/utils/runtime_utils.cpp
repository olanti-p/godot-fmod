...
void RuntimeUtils::_bind_methods() {
    ...
    ClassDB::bind_static_method(
        "RuntimeUtils",
        D_METHOD("to_3d_attributes_transform_physicsbody", "attributes", "transform", "physicsbody"),
...
void RuntimeUtils::to_3d_attributes_node(Ref<FmodTypes::FMOD_3D_ATTRIBUTES> attributes, Object* node, Object* physicsbody) {
...
void RuntimeUtils::to_3d_attributes_transform_physicsbody(Ref<FmodTypes::FMOD_3D_ATTRIBUTES> attributes, const Variant& transform, Object* physicsbody) {
    ...
    if (transform_type == Variant::TRANSFORM3D) {
        ...
        UtilityFunctions::push_warning(
            "[FMOD] Converting to 3D Attributes is only available for Transform3D and Transfom2D. ", __FUNCTION__, __FILE__, __LINE__
        );
    }
}
...
void RuntimeUtils::to_3d_attributes(Ref<FmodTypes::FMOD_3D_ATTRIBUTES> attributes, const Variant& position) {
    ...
    switch (position.get_type()) {
        ...
        default: {
            UtilityFunctions::push_warning(
                "[FMOD] Converting to 3D Attributes is only available for Transform3D, Transform2D, Vector3 and Vector2.", __FUNCTION__, __FILE__, __LINE__
            );
            break;
        }
    }
